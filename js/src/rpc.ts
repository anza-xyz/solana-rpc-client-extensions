...
export async function getStakeActivation(rpc: Rpc<SolanaRpcApi>, stakeAddress: Address): Promise<StakeActivation> {
    ...
    const stakeHistoryAccount = await fetchEncodedAccount(rpc, SYSVAR_STAKE_HISTORY_ADDRESS);
    ...
    const {
        ...
        activating,
        deactivating
    } = getStakeActivatingAndDeactivating(stake.data.stake.delegation, epochInfo.epoch, stakeHistory.data);
